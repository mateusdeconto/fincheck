    ...
    <div className="animate-slide-up space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2.5">
        <button
          ...
          className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl font-semibold text-white text-sm active:scale-[0.99] transition-all"
        ...
        <a
          href={(() => {
            const msg = buildWhatsAppMessage(businessData, financialData, diagnosis, metrics, healthStatus);
            const subject = `Diagnóstico FinCheck — ${businessData.businessName}`;
            return `mailto:?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(msg)}`;
          })()}
          className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl font-semibold text-ink-700 text-sm bg-white border border-ink-200 hover:bg-ink-50 transition-colors"
        >
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
