...
const SECTOR_EXTRA = {
  ...
  varejo:      { fixedCostPct: [18, 28], laborPct: [10, 18], rentPct: [6, 10],  breakEvenPct: [72, 88], survivalRate2y: 48 },
  servicos:    { fixedCostPct: [22, 35], laborPct: [30, 45], rentPct: [3, 8],   breakEvenPct: [55, 72], survivalRate2y: 56 },
  saude:       { fixedCostPct: [28, 40], laborPct: [32, 48], rentPct: [5, 10],  breakEvenPct: [58, 75], survivalRate2y: 60 },
  beleza:      { fixedCostPct: [28, 40], laborPct: [35, 50], rentPct: [6, 10],  breakEvenPct: [60, 78], survivalRate2y: 54 },
  tecnologia:  { fixedCostPct: [30, 48], laborPct: [40, 60], rentPct: [2, 5],   breakEvenPct: [50, 68], survivalRate2y: 62 },
  construcao:  { fixedCostPct: [10, 22], laborPct: [28, 42], rentPct: [1, 4],   breakEvenPct: [75, 90], survivalRate2y: 44 },
  educacao:    { fixedCostPct: [32, 48], laborPct: [35, 50], rentPct: [5, 12],  breakEvenPct: [62, 80], survivalRate2y: 55 },
  industria:   { fixedCostPct: [15, 28], laborPct: [20, 35], rentPct: [3, 8],   breakEvenPct: [70, 88], survivalRate2y: 50 },
...
function BenchmarkPremium({ macroData, segment, plan, onUpgrade }) {
  ...
  const freshLabel = minutesAgo !== null
    ? (minutesAgo < 1 ? 'agora mesmo' : `${minutesAgo} min atrás`)
    : null;
  const isLive = macroData && !macroData.selic?.isFallback;
    ...
    <div>
      ...
      <div className="mb-5">
        <p className="text-[10px] font-bold text-ink-400 uppercase tracking-wider mb-3">
          Benchmarks setoriais — SEBRAE
        </p>
        <div className="grid grid-cols-2 gap-2">
          {[
            { label: 'Desp. fixas típicas', value: `${extra.fixedCostPct[0]}–${extra.fixedCostPct[1]}% da receita` },
            { label: 'Custo de pessoal',    value: `${extra.laborPct[0]}–${extra.laborPct[1]}% da receita` },
            { label: 'Aluguel típico',      value: `${extra.rentPct[0]}–${extra.rentPct[1]}% da receita` },
            { label: 'Ponto de equilíbrio', value: `${extra.breakEvenPct[0]}–${extra.breakEvenPct[1]}% da capacidade` },
          ].map(row => (
            <div key={row.label} className="bg-ink-50 rounded-lg p-3">
              <p className="text-[10px] text-ink-400 mb-1">{row.label}</p>
              <p className="text-sm font-bold text-ink-800">{row.value}</p>
            </div>
          ))}
        </div>
        <div className="mt-2 flex items-center gap-2 bg-brand-50 border border-brand-100 rounded-lg px-3 py-2">
          ...
          <p className="text-[11px] text-brand-700">
            <strong>{extra.survivalRate2y}%</strong> das empresas do setor sobrevivem 2 anos — fonte SEBRAE
          </p>
        </div>
      </div>

      {/* Divisor */}
      <div className="border-t border-ink-100 mb-5" />

      {/* Sub-seção 2: BCB ao vivo */}
      <div>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-1.5">
            <span className={`w-2 h-2 rounded-full ${isLive ? 'bg-money-500 animate-pulse' : 'bg-amber-400'}`} />
            <p className="text-[10px] font-bold text-ink-400 uppercase tracking-wider">
              Macro ao vivo — Banco Central
            </p>
          </div>
          {freshLabel && (
            <span className="text-[10px] text-ink-400">
              {isLive ? `atualizado ${freshLabel}` : 'estimado'}
            </span>
          )}
        </div>
        <div className="grid grid-cols-3 gap-2">
          {[
            {
              label: 'Selic a.a.',
              value: `${macroData?.selic?.value ?? '—'}%`,
              sub:   macroData?.selic?.value ? `≈ ${(parseFloat(macroData.selic.value) / 12).toFixed(2)}%/mês` : '',
            },
            {
              label: 'IPCA',
              value: `${macroData?.ipca?.value ?? '—'}%`,
              sub:   'inflação atual',
            },
            {
              label: 'USD/BRL',
              value: `R$ ${macroData?.usdBrl?.value ?? '—'}`,
              sub:   'câmbio hoje',
            },
          ].map(item => (
            <div key={item.label} className="bg-ink-50 rounded-lg p-3">
              <p className="text-[10px] text-ink-400 mb-1">{item.label}</p>
              <p className="text-base font-bold text-ink-900 font-mono">{item.value}</p>
              {item.sub && <p className="text-[10px] text-ink-400 mt-1">{item.sub}</p>}
            </div>
          ))}
        </div>
        <p className="text-[10px] text-ink-400 mt-2">
          Esses dados foram usados para enriquecer sua análise · Fonte: BCB / AwesomeAPI
        </p>
      </div>
    </div>
  );
    ...
    return (
      <div className="card p-5 relative overflow-hidden">
        ...
        <div className="blur-sm pointer-events-none select-none">
          ...
          <div className="grid grid-cols-2 gap-2 mb-4">
            {['Desp. fixas', 'Pessoal', 'Aluguel', 'P. equilíbrio'].map(l => (
              <div key={l} className="bg-ink-50 rounded-lg p-3">
                <p className="text-[10px] text-ink-400 mb-1">{l}</p>
                <p className="text-sm font-bold text-ink-300">—–—% da receita</p>
              </div>
            ))}
          </div>
          <div className="grid grid-cols-3 gap-2">
            {['Selic a.a.', 'IPCA', 'USD/BRL'].map(l => (
              <div key={l} className="bg-ink-50 rounded-lg p-3">
                <p className="text-[10px] text-ink-400 mb-1">{l}</p>
                <p className="text-base font-bold text-ink-300 font-mono">–––%</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/75 backdrop-blur-[2px] rounded-xl">
          ...
          <button
            onClick={onUpgrade}
            className="text-xs font-bold bg-ink-900 text-white px-4 py-2 rounded-lg hover:bg-ink-800 transition-colors"
          >
            Desbloquear PRO
          </button>
        </div>
